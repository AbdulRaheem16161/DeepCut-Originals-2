...
const YoutubeIcon = () => <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor">
    <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
  </svg>;
const LinkedInIcon = () => <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor">
    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
  </svg>;
const FiverrIcon = () => <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor">
...
const UpworkIcon = () => <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor">
...
const SocialButton = ({
  ...
  return <Button variant="outline" size="sm" className={`gap-2 transition-colors ${colors[type]}`} onClick={() => window.open(url, '_blank')}>
      {icons[type]}
      <span className="hidden sm:inline">{labels[type]}</span>
    </Button>;
};
const MeetTheTeam = () => {
  return <section id="team" className="py-20 px-4 bg-muted/30">
      <div className="container mx-auto max-w-6xl">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-orbitron font-bold mb-4 text-foreground">
            Meet The <span className="text-[#dfa620]">Team</span>
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {teamMembers.map(member => <Card key={member.name} className="hover:shadow-lg transition-shadow duration-300 border-border/50 bg-card/50 backdrop-blur-sm">
              <CardContent className="p-6 flex flex-col items-center text-center space-y-4">
                ...
                {/* Name and Role */}
                <div>
                  <h3 className="text-xl font-semibold font-orbitron mb-1 text-[#dfa920]">
                    {member.name}
                  </h3>
                ...
                {/* Social Links */}
                {Object.keys(member.socials).length > 0 && <div className="flex flex-wrap justify-center gap-2 pt-2">
                    ...
                    {member.socials.upwork && <SocialButton type="upwork" url={member.socials.upwork} />}
                  </div>}
              </CardContent>
            </Card>)}
        </div>
